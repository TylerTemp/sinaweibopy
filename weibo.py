...
class JsonDict(dict):
    ...
    def __getattr__(self, attr):
        try:
            ...
            raise AttributeError("'JsonDict' object has no attribute '%s'" %
                                 attr)
...
def _encode_params(**kw):
    ...
    for k, v in kw.items():
        if isinstance(v, basestring):
            qv = v.encode('utf-8') if is_unicode(v) else v
            args.append('%s=%s' % (k, quote(qv)))
        elif isinstance(v, collections.Iterable):
            for i in v:
                qv = i.encode('utf-8') if is_unicode(i) else str(i)
                args.append('%s=%s' % (k, quote(qv)))
        else:
            qv = str(v)
            args.append('%s=%s' % (k, quote(qv)))
...
def _encode_multipart(**kw):
    ...
    for k, v in kw.items():
        ...
        if hasattr(v, 'read'):
            ...
            data.append(v.encode('utf-8') if is_unicode(v) else v)
    data.append('--%s--\r\n' % boundary)
    return '\r\n'.join(data), boundary
...
def _http_upload(url, authorization=None, **kw):
    logger.info('MULTIPART POST %s' % url)
...
class APIClient(object):
    ...
    def parse_signed_request(self, signed_request):
        ...
        logger.info('parse signed request: %s' % sr)
        ...
        if data['algorithm'] != 'HMAC-SHA256':
            return None
        expected_sig = hmac.new(self.client_secret,
                                enc_payload,
...
class _Executable(object):
    ...
    def __call__(self, **kw):
        ...
        if method == _HTTP_POST and 'pic' in kw:
            method = _HTTP_UPLOAD
        return _http_call('%s%s.json' % (self._client.api_url, self._path),
                          method,
